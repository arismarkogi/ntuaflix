...
function validateParameters(scope, params) {
  ...
  for (const requiredParam of requiredParams) {
    if (!params[requiredParam]) {
      ...
      process.exit(1);
    } else if (!requiredParam.startsWith('--')) {
      console.error(`Invalid required parameter format for scope ${scope}. Required parameters should start with '--'.`);
      process.exit(1);
    }
  }
}
...
const [command,scope, ...rest] = args; // Η πρώτη παράμετρος είναι το scope, το υπόλοιπο είναι παράμετροι
